...
bool eval2 (string path2ResultPoses, int32_t sequence, Mail* mail) {

  // ground truth and result directories
  string gt_dir         = "./dataset/poses";
  // string res_path       = path2ResultPoses;

  // create output directories
  char results_dir[256];
  sprintf(results_dir,"./results-%d", sequence);
  ...
  system(("mkdir " + std::string(results_dir)).c_str());
  system(("mkdir " + error_dir).c_str());
  system(("mkdir " + plot_path_dir).c_str());
  system(("mkdir " + plot_error_dir).c_str());
  
  system(("cp " + path2ResultPoses + " " + std::string(results_dir)).c_str());
  ...
  // file name
  char file_name[256];
  sprintf(file_name,"%02d.txt",sequence);
  
  // read ground truth and result poses
  vector<Matrix> poses_gt     = loadPoses(gt_dir + "/" + file_name);
  vector<Matrix> poses_result = loadPoses(path2ResultPoses);
  ...
  savePathPlot(poses_gt,poses_result,plot_path_dir + "/" + file_name);
  vector<int32_t> roi = computeRoi(poses_gt,poses_result);
  plotPathPlot(plot_path_dir,roi,sequence);
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  // run evaluation
  bool success = eval2(result_sha,atoi(argv[2]),mail);
